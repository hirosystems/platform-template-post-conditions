...
(define-fungible-token good-token)
...
(define-constant TOKEN_NAME "Good Token")
(define-constant TOKEN_SYMBOL "GT")
...
(define-public (mint (amount uint))
  (ft-mint? good-token amount tx-sender)
)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? good-token amount sender recipient))
    (match memo
...
(begin
  (try! (as-contract (mint u500000000)))
  (as-contract (transfer u500000000 tx-sender 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM none))
)
